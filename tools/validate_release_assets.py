"""Static release-asset validation for the AST audio classification DIMER pipeline.
...
EXPECTED_MODEL_ID = "MIT/ast-finetuned-audioset-10-10-0.4593"
...
CODE_MARKERS = (
    'from ast_audio_classification_pipeline import MAX_AUDIO_SECONDS, MAX_INPUT_SECONDS, NUM_LABELS, '
    'SAMPLE_RATE',
    'from ast_audio_classification_pipeline import MODEL_ID, MODEL_KEY, MODEL_REVISION, '
    'ASTAudioClassificationPipeline, stage_missing_files, verify_snapshot',
    ...
    "print({'ceilings': {'SAMPLE_RATE': SAMPLE_RATE, 'MAX_AUDIO_SECONDS': MAX_AUDIO_SECONDS, "
    "'MAX_INPUT_SECONDS': MAX_INPUT_SECONDS, 'NUM_LABELS': NUM_LABELS}})",
...
MARKDOWN_MARKERS = (
    '**Capability:** multi-label audio event classification over the 527 AudioSet labels',
    '**No adaptation occurs:**',
    'Each `score` is an **independent sigmoid**',
    '**not a calibrated probability**',
    '**No decision threshold is shipped**',
    '**ordered by descending score**',
    'No metric is reported in this notebook: the repository ships no metric helper and no labelled audio',
    '**only its first 10.24 s reach the model**',
    'sanity evidence only',
    'speech transcription, speaker identification, temporal localisation of events inside the window',
)
