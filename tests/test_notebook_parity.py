...
The notebook carries `src/<package>/pipeline.py` (and any sibling modules) verbatim; these tests
fail whenever the carried cells, the inline manifest, or the inline pins diverge from the repository.
"""
...
def test_par1_embedded_modules_equal_repository_modules(notebook: dict) -> None:
    ...
    for cell, module, rel in zip(tagged, ctx["modules"], ctx["module_rels"], strict=True):
